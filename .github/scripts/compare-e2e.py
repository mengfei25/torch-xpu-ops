...
- Optional `--markdown` argument to generate two GitHub-flavored Markdown files:
  * `{basename}_summary.md` - overall summary table.
  * `{basename}_details.md` - detailed per-suite overview, new failures, improvements.
"""
...
# ----------------------------------------------------------------------
# Markdown output functions (NEW: split into summary and details)
# ----------------------------------------------------------------------
def write_summary_markdown(combined_summary, details_df, filename):
    """
    Write a Markdown file containing only the Overall Summary table.
    """
...
def write_details_markdown(details_df, filename):
    """
    Write a Markdown file with:
    - Overview by Suite (plain table with emoji indicators)
    - New Failures & Regressions (HTML tables with row colours)
    - New Passes & Improvements (HTML tables with row colours)
    """
    if details_df.empty:
        with open(filename, 'w', encoding='utf-8') as f:
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        # ----- Overview by Suite (plain Markdown, with emoji indicators) -----
        if 'suite' in details_df.columns:
            ...
            for suite in details_df['suite'].dropna().unique():
                ...
                # Accuracy counts (change analysis)
                acc_fail = 0
                ...
                def geomean(series):
                    vals = series.dropna()
                    vals = vals[vals > 0]  # only positive ratios
                    if len(vals) == 0:
                        return None
                    return np.exp(np.log(vals).mean())
            ...
            if suite_rows:
                ...
                # Helper to add emoji to ratio based on threshold
                def fmt_ratio(val):
                    if val and val != "":
                        try:
                            num = float(val)
                ...
                for s in suite_rows:
                    row = [
                        ...
                        fmt_ratio(s['ind_ratio']),
                        fmt_ratio(s['eag_ratio']),
                    ]
                    f.write("| " + " | ".join(row) + " |\n")
                f.write("\n")
            else:
...
# ----------------------------------------------------------------------
# Main (updated for split markdown)
# ----------------------------------------------------------------------
def main():
...
Examples:
  ...
  %(prog)s --target_dir --baseline_dir --comparison.xlsx --markdown report.md   # Also generates report_summary.md and report_details.md
        """
    )
    ...
    parser.add_argument("-m", "--markdown", help="Base name for Markdown reports (e.g., report.md -> report_summary.md, report_details.md)")
    ...
    if args.markdown:
        ...
        write_summary_markdown(combined_summary, details, summary_file)
        write_details_markdown(details, details_file)
